...
SCENE_PATTERNS = [
    # 标准格式：INT. / EXT. 开头
    r'^INT\.\s+', r'^EXT\.\s+',
    # 无点号格式：INT  / EXT  后跟空格（侏罗纪公园等）
    r'^INT\s+', r'^EXT\s+',
    # 组合格式：INT/EXT, EXT/INT, INT./ EXT. （泰坦尼克号等）
    r'^INT/EXT', r'^EXT/INT', r'^INT\./\s*EXT', r'^EXT\./\s*INT',
    # INT/ 和 EXT/ 开头
    r'^INT/', r'^EXT/',
    # 数字前缀 + 可选点号 + 可选空格 + INT/EXT
    # "1 INT -- CABIN -- NIGHT (1946)"（肖申克）
    # "1    INT. DERELICT HOUSE - DAY"（记忆碎片）
    # "2 EXT./ INT. MIR ONE"（泰坦尼克号）
    r'^\d+\.?\s*(INT|EXT|INT\./EXT|EXT\./INT)',
    # INT -- / EXT -- 格式（肖申克救赎专用）
    r'^(INT|EXT)\s+--',
    # SCENE 关键词
    r'^SCENE\s+\d+',
    # INTERIOR / EXTERIOR 全称
    r'^INTERIOR\s+', r'^EXTERIOR\s+',
    # 纯数字编号场景（Titanic 的 "1 BLACKNESS" — 最后兜底，避免误匹配）
    r'^\d+\.?\s+[A-Z][A-Z\s\.]{2,}',
]
